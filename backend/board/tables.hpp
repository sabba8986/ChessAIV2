...
namespace tables{
    ...
    namespace attacks{
        constexpr std::size_t size(PieceType p){
            switch(p){
        ...
        constexpr std::array<std::uint64_t, size(p)> populate_table(){
            using namespace bitboard;
            std::array<std::uint64_t, size(p)> table{};
            auto dirs = get_directions(p);
            auto dir1 = dirs[0], dir2 = dirs[1], dir3 = dirs[2], dir4 = dirs[3];
            int i = 0;
            for(std::uint64_t pos = 1; pos; pos <<= 1, i++){
                const auto magic_info = (p == PieceType::ROOK) ? magics::rook[i] : magics::bishop[i]; 
        ...
        template<>
        constexpr std::array<std::uint64_t, size(PieceType::KING)> populate_table<PieceType::KING>(){
            using namespace bitboard;
            std::array<std::uint64_t, 64> table{};
